...
# ── Helper: get API key from user header or env ───────────────────────────────
def get_effective_api_key():
    """Use user-provided key from header, fallback to server env key."""
...
def ai_status():
    user_key = request.headers.get('X-User-Api-Key', '').strip()
    if user_key and user_key.startswith('sk-ant-'):
        return jsonify({"configured": True, "model": "claude-sonnet-4-6", "source": "user_key"})
...
def ai_upload():
    ...
    try:
        ...
        if ext == 'pdf':
            ...
            orig_key3 = os.environ.get("ANTHROPIC_API_KEY","")
        user_key3 = request.headers.get('X-User-Api-Key','').strip()
        if user_key3 and user_key3.startswith('sk-ant-'):
            os.environ["ANTHROPIC_API_KEY"] = user_key3
        result = analyze_uploaded_file(filename, content, ext, question)
        if user_key3 and user_key3.startswith('sk-ant-'):
            os.environ["ANTHROPIC_API_KEY"] = orig_key3
        return jsonify(result)
...
def ai_pharmacogenomics():
    ...
    user_key2 = request.headers.get('X-User-Api-Key','').strip()
    if user_key2 and user_key2.startswith('sk-ant-'):
        os.environ["ANTHROPIC_API_KEY"] = user_key2
